...
class Adm_contact extends Backend_Controller {
	...
	{	
		...
		//set view
		$this->stencil->paint('contact/list_contact',$data);
        /*$this->load->library('breadcrumb');
	...
	function grab_data_contact()
	{
		...
		foreach ($query as $hasil) {
			...
			$output['data'][]=array(
				...
				$hasil['conNama'],
				$hasil['conEmail'],
				anchor("#myModal",'<span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span> Lihat Pesan', array('title'=>"Lihat Pesan $nama" , 'class'=>'btn btn-danger btn-sm', 'id'=>'custId', 'data-toggle'=>'modal', 'data-id'=>"$id", "data-target"=>"#myModal"))." ".				
