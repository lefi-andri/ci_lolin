...
class Produk extends Backend_Controller {
	...
	public function index()
	{
		...
		//set view
		$this->stencil->paint('produk/list_produk', $data);
	...
	public function grab_data_produk()
	{
		...
		foreach ($query as $hasil) {
			...
			$output['data'][]=array(
				...
				"Rp. ".$hasil['prodsPrice'],
				anchor("admin/product/edit/$id",'<span class="glyphicon glyphicon-edit" aria-hidden="true"></span> Edit', array('title'=>"Edit $nama" , 'class'=>'btn btn-default btn-sm'))." ".
				anchor("admin/product/delete/$id",'<span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Delete', array('title'=>"Delete $nama" , 'class'=>'btn btn-default btn-sm', 'onclick' => "return confirm('Anda yakin ingin menghapus data $nama ?')"))
			);
		$nomor_urut++;
		}
	...
	*/
	...
	{
		//set title
		$this->stencil->title('Tambah Produk');
		//set layout
		$this->stencil->layout('backend_layout');
		//set css
		//$this->stencil->css('bootstrap/bootstrap.min');
		...
		$this->data = array(
			'label'			=> 'Tambah produk',
			'form_action' 	=> 'backend/produk/add_product',
		...
		if (isset($_POST['submit'])) {
			if ($this->form_validation->run() == FALSE) {
				...
				//set view
				$this->stencil->paint('produk/form_add_produk',$this->data);
	...
	{
		//set title
		$this->stencil->title('Edit Produk');
		//set layout
		$this->stencil->layout('backend_layout');
		//set css
		//$this->stencil->css('bootstrap/bootstrap.min');
		...
		if (empty($id)) {
			...
			if (isset($_POST['submit'])) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					//$this->load->view('include/template', $this->data);
					//set view
					$this->stencil->paint('produk/form_edit_produk',$this->data);
				}
