	...
	public function grab_data_kategori_produk()
	{
		...
		foreach ($query as $hasil) {
			...
			$output['data'][]=array(
				...
				anchor("admin/product/kategori/remove/$id",'<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> Hapus Gambar', array('title'=>"Hapus gambar navbar $nama" , 'class'=>'btn btn-warning btn-xs', 'onclick' => "return confirm('Anda yakin ingin menghapus gambar navbar $nama ?')"))." ".
				anchor("admin/product/kategori/edit/$id",'<span class="glyphicon glyphicon-edit" aria-hidden="true"></span> Edit', array('title'=>"Edit $nama" , 'class'=>'btn btn-dark btn-xs'))." ".
				anchor("admin/product/kategori/delete/$id",'<span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Delete', array('title'=>"Delete $nama" , 'class'=>'btn btn-danger btn-xs', 'onclick' => "return confirm('Anda yakin ingin menghapus data $nama ?')"))
			);
		$nomor_urut++;
		}
