...
class Item_order extends Backend_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('item_order_model', 'models');
	}
	...
	public function add_item_order()
	{
		...
		$this->data = array(
			'main_view' 		=> 'item_order/form_add_item_order',
	...
	public function check_orders()
	{
		if (count($_POST['product']) == 0) {
	        ...
	        $data = array(
	            ...
	            'main_view'         => 'item_order/list_input_jumlah_pembelian',
	        );
	...
	function cek_harga($id_produk, $harga_produk, $jumlah_order){
		...
		/*if ($data_produk->num_rows() > 0) {
			$data = ($data_produk->harga_jumlah_unit * $jumlah_order);
			return $data;
		}else{
			$data = ($harga_produk * $jumlah_order);
			return $data;
		}*/

		/*foreach ($data_produk->result() as $value) {

			if ($jumlah_order == $value->jumlah_unit) {
				#$harga_diskon = ($jumlah_order * $value->harga_jumlah_unit);
				#$harga_diskon = "ada";
				$harga_diskon += $nilai;
			} else {
				#$harga_diskon = ($harga_produk * $jumlah_order);
				$harga_diskon = 0;
			}
	...
	public function confirmation_order(){
		...
		if ($this->input->post('prodsId[]') == "") {
	        ...
	        $data = array(
	            'main_view'         => 'item_order/list_order_confirmation',
	...
	public function simpan_order()
	{
		if (!$this->session->userdata('reseller_id')) {
			...
			if ($data) {
				...
				$array_items = array('id_reseller', 'order_code', 'jumlah', 'id_produk');
	...
	public function edit_item_order($id)
	{
		...
		$this->data = array(
			'main_view' 		=> 'item_order/form_edit_item_order',
			#'form_action' 		=> 'admin/reseller/order/item/edit/'.$id
		);
		...
		if (empty($id)) {
			...
			/*if (isset($_POST['submit'])) {
				...
				$this->form_validation->set_rules('jumlah', 'Jumlah Order', 'trim|required|xss_clean');
	...
	public function update_diskon(){
			...
			$this->data = array(
				'main_view' 		=> 'item_order/list_edit_order_confirmation',
				#'form_action' 		=> 'admin/reseller/order/item/edit/'.$id
			);
	...
	public function simpan_update(){
		...
		if (isset($_POST['submit'])) {
				...
				redirect(base_url('admin/reseller/order/'),'refresh');
...
/* End of file Item_order.php */
/* Location: ./application/modules/backend/controllers/Item_order.php */