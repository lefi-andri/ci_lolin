...
class Kedaluwarsa_reseller_organisasi extends Backend_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('kedaluwarsa_reseller_organisasi_model', 'models');
	...
	public function index()
	{
		...
		foreach ($users as $value) {
			...
			$this->table->add_row(array(
				...
				anchor(base_url().'backend/kedaluwarsa_reseller_organisasi/backup_data/'.$value->id, '<span class="glyphicon glyphicon-file" aria-hidden="true"></span> Check', array('class'=>'btn btn-info btn-xs')),
				($selisih <= 0)? '<span class="label label-danger">Kedaluwarsa</span>': '',
				($selisih <= 0)? anchor(base_url().'backend/kedaluwarsa_reseller_organisasi/backup_data/'.$value->id, '<span class="glyphicon glyphicon-file" aria-hidden="true"></span> Backup Data', array('class'=>'btn btn-warning btn-xs')) : '',
				($selisih <= 0)? anchor(base_url().'backend/kedaluwarsa_reseller_organisasi/delete_data/'.$value->id, '<span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Hapus Data', array('class'=>'btn btn-danger btn-xs', 'title'=>'Hapus data reseller', 'onclick' => "return confirm('Anda yakin ingin menghapus reseller ?')")) : '',
			));
			$no++;
		}
		...
		$this->data = array(
			'main_view' 	=> 'kedaluwarsa_reseller_organisasi/list_kedaluwarsa',
	...
	public function backup_data($id = NULL){
		...
		$this->load->view('kedaluwarsa_reseller_organisasi/list_backup_data', $this->data);
	}
	...
	public function delete_data($id = NULL){
        ...
        redirect('backend/kedaluwarsa_reseller_organisasi/index');
	}

}

/* End of file Kedaluwarsa_reseller_organisasi.php */
/* Location: ./application/modules/backend/controllers/Kedaluwarsa_reseller_organisasi.php */